...
const ClassroomsList = () => {
  ...
  const rooms = useMemo(() => classroomsApi.getAllSync(), []);
    ...
    >
      ...
      <div className="mb-4 flex shrink-0 flex-wrap items-end gap-3">
        <SearchField aria-label="搜索教室" value={searchQuery} onChange={handleSearchChange}>
          <SearchField.Group><SearchField.SearchIcon /><SearchField.Input className="w-[200px]" placeholder="搜索教室名称或所在楼栋..." /><SearchField.ClearButton /></SearchField.Group>
        </SearchField>
        <Select className="w-40" aria-label="楼栋" placeholder="全部楼栋" value={buildingFilter} onChange={(k) => { setBuildingFilter(k); setPage(1); }}>
          <Label>楼栋</Label><Select.Trigger><Select.Value /><Select.Indicator /></Select.Trigger>

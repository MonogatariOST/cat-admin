...
const ClassesList = () => {
  ...
  const classes = useMemo(() => classesApi.getAllSync(), []);
    ...
    >
      ...
      <div className="mb-4 flex shrink-0 flex-wrap items-end gap-3">
        <SearchField aria-label="搜索班级" value={searchQuery} onChange={handleSearchChange}>
          <SearchField.Group><SearchField.SearchIcon /><SearchField.Input className="w-[200px]" placeholder="搜索班级名称、年级或专业..." /><SearchField.ClearButton /></SearchField.Group>
        </SearchField>
        <Select className="w-[140px]" aria-label="年级" placeholder="全部年级" value={gradeFilter} onChange={(k) => { setGradeFilter(k); setPage(1); }}>
          <Label>年级</Label><Select.Trigger><Select.Value /><Select.Indicator /></Select.Trigger>

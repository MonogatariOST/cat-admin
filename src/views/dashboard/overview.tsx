      ...
      <svg width={width} height={CHART_HEIGHT}>
        ...
        <Group left={CHART_MARGIN.left} top={CHART_MARGIN.top}>
          ...
          {activityData.map(d => {
            ...
            return (
              <Bar key={d.label}
                x={xScale(d.label)} y={yScale(d.value)}
                width={xScale.bandwidth()} height={yMax - yScale(d.value)}
          ...
          {hoveredLabel !== null && activityData.filter(d => d.label === hoveredLabel).map(d => (
            <text key={`tip-${d.label}`}
              x={(xScale(d.label) ?? 0) + xScale.bandwidth() / 2}
              y={yScale(d.value) - 8} textAnchor="middle"
              fill="var(--accent)" fontSize={13} fontWeight={700}>
              {d.value}
            </text>
          ))}
          {activityData.map(d => (
...
const DashboardOverview = () => {
  useSetPageTitle('看板概览', '欢迎使用 HeroAdmin');
  return (
    <BuiPage>
      ...
      {[
        ...
        <Card key={stat.label} className="p-4">
          <Card.Content>
            ...
            <p className="mt-1 text-2xl font-semibold text-foreground font-price">{stat.value}</p>
            <p className={`mt-1 text-sm ${stat.change.startsWith('+') ? 'text-success' : stat.change.startsWith('-') ? 'text-danger' : 'text-foreground'}`}>
              {stat.change}
            </p>
          </Card.Content>
        </Card>
    ...
    <div className="mt-8 grid grid-cols-1 gap-6 lg:grid-cols-2">
      <Card className="p-6">
        ...
        <Card.Content><ActivityBarChart /></Card.Content>
      </Card>
      <Card className="p-6">
        <Card.Header>
        ...
        <Card.Content>
          <div className="grid grid-cols-2 gap-3">
            <Button variant="primary" className="w-full justify-start gap-2"><UserPlus size={18} />新建用户</Button>
            <Button variant="outline" className="w-full justify-start gap-2"><FileDown size={18} />导出报表</Button>
            <Button variant="outline" className="w-full justify-start gap-2"><Send size={18} />发送通知</Button>
            <Button variant="outline" className="w-full justify-start gap-2"><Settings size={18} />系统设置</Button>
          </div>
        </Card.Content>
      </Card>
    </div>

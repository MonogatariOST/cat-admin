...
import {
  ...
  TableCellsIcon as TableCellsOutline,
    ...
    items: [
      {
        ...
        icon: (active) =>
          active ? (
            <Squares2X2Solid className="size-5" />
          ) : (
            <Squares2X2Outline className="size-5" />
          ),
      },
      {
        label: "分析",
        ...
        icon: (active) =>
          active ? (
            <ChartBarSquareSolid className="size-5" />
          ) : (
            <ChartBarSquareOutline className="size-5" />
          ),
      },
    ],
  },
    ...
    items: [
      {
        ...
        icon: (active) =>
          active ? (
            <BookOpenSolid className="size-5" />
          ) : (
            <BookOpenOutline className="size-5" />
          ),
      },
      {
        label: "老师管理",
        ...
        icon: (active) =>
          active ? (
            <UserGroupSolid className="size-5" />
          ) : (
            <UserGroupOutline className="size-5" />
          ),
      },
      {
        label: "班级管理",
        ...
        icon: (active) =>
          active ? (
            <RectangleStackSolid className="size-5" />
          ) : (
            <RectangleStackOutline className="size-5" />
          ),
      },
      {
        label: "教室管理",
        ...
        icon: (active) =>
          active ? (
            <BuildingOffice2Solid className="size-5" />
          ) : (
            <BuildingOffice2Outline className="size-5" />
          ),
      },
    ],
  },
    ...
    items: [
      {
        ...
        icon: (active) =>
          active ? (
            <DocumentPlusOutline className="size-5 text-accent" />
          ) : (
            <DocumentPlusOutline className="size-5 text-foreground-400" />
          ),
      },
    ],
  },
    ...
    items: [
      {
        ...
        icon: (active) =>
          active ? (
            <TableCellsOutline className="size-5 text-accent" />
          ) : (
            <TableCellsOutline className="size-5 text-foreground-400" />
          ),
      },
      {
        label: "高级表单",
        ...
        icon: (active) =>
          active ? (
            <ListBulletOutline className="size-5 text-accent" />
          ) : (
            <ListBulletOutline className="size-5 text-foreground-400" />
          ),
      },
      {
        label: "任务看板",
        ...
        icon: (active) =>
          active ? (
            <RectangleGroupOutline className="size-5 text-accent" />
          ) : (
            <RectangleGroupOutline className="size-5 text-foreground-400" />
          ),
      },
      {
        label: "审计日志",
        ...
        icon: (active) =>
          active ? (
            <ClockOutline className="size-5 text-accent" />
          ) : (
            <ClockOutline className="size-5 text-foreground-400" />
          ),
      },
    ],
  },
    ...
    items: [
      {
        ...
        icon: (active) =>
          active ? (
            <DocumentTextSolid className="size-5" />
          ) : (
            <DocumentTextOutline className="size-5" />
          ),
      },
      {
        label: "404 \u9875\u9762",
        ...
        icon: (active) =>
          active ? (
            <MagnifyingGlassSolid className="size-5" />
          ) : (
      ...
      },
      {
        ...
        icon: (active) =>
          active ? (
            <LockClosedSolid className="size-5" />
          ) : (
            <LockClosedOutline className="size-5" />
          ),
      },
      {
        label: "500 \u9875\u9762",
        ...
        icon: (active) =>
          active ? (
            <ExclamationSolid className="size-5" />
          ) : (
            <ExclamationOutline className="size-5" />
          ),
      },
    ],
  },
    ...
    items: [
      {
        ...
        icon: (active) =>
          active ? (
            <ShieldCheckSolid className="size-5" />
          ) : (
            <ShieldCheckOutline className="size-5" />
          ),
      },
      {
        label: "权限管理",
      ...
      },
      {
        ...
        icon: (active) =>
          active ? (
            <Cog6ToothSolid className="size-5" />
          ) : (
            <Cog6ToothOutline className="size-5" />
          ),
      },
    ],
  },
    ...
    >
      {navGroups.map((group) => {
        const groupKey =
          group.title === "系统管理"
            ? "system"
            : (group.items[0]?.path?.split("/")[1] ?? group.title);
        return (
            ...
            <Accordion.Heading>
              <Accordion.Trigger className="flex items-center gap-2 px-3 py-2 text-xs font-medium uppercase tracking-wider text-foreground-400 hover:bg-surface-tertiary rounded-lg">
                ...
                <Accordion.Indicator className="ml-auto">
                  <ChevronDownOutline className="size-[14px]" />
                </Accordion.Indicator>
              </Accordion.Trigger>
            </Accordion.Heading>
            <Accordion.Panel>
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden">
      ...
      <div className="flex h-16 items-center gap-3 px-5">
        ...
        <span className="text-lg font-semibold text-foreground">CatAdmin</span>
      </div>
      ...
      <NavLink
        to="/profile"
        onClick={onNavClick}
        className="flex items-center gap-3 px-4 py-3 hover:bg-surface-secondary rounded-lg transition-colors"
      >
...
const MainLayout = () => {
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-background">
      ...
      >
        <div
          className={`flex-1 overflow-hidden ${isSidebarCollapsed ? "hidden" : "block"}`}
        >
          <SidebarContent />
        </div>
        {isSidebarCollapsed && (
        ...
        <header className="flex flex-col bg-surface">
          <div className="flex h-16 items-center gap-4 px-4 sm:px-6">
            ...
            <div className="hidden sm:flex relative flex-1 max-w-md">
              <MagnifyingGlassOutline className="size-4 absolute left-3 top-1/2 -translate-y-1/2 text-foreground-400" />
              <input
                aria-label="搜索"
                type="text"
                placeholder="搜索..."
            ...
            >
              {isDark ? <SunOutline className="size-[18px]" /> : <MoonOutline className="size-[18px]" />}
            </Button>
              ...
              <Dropdown.Popover placement="bottom end">
                ...
                >
                  <Dropdown.Item id="settings" textValue="账号设置">
                    <Cog6ToothOutline className="size-4 shrink-0 text-muted" />
                    <Label>账号设置</Label>
                  </Dropdown.Item>
                  <Dropdown.Item
                    id="logout"
                    ...
                    variant="danger"
                  >
                    <ArrowRightStartOnRectangleOutline className="size-4 shrink-0 text-danger" />
                    <Label>退出登录</Label>
                  </Dropdown.Item>
                </Dropdown.Menu>
              </Dropdown.Popover>
